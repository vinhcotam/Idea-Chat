...
class MessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        database.collection(KEY_COLLECTION_USERS).document(user.id)
            .get()
            .addOnSuccessListener { document ->
                if (document != null) {
                    user.image = document.data?.get(KEY_IMAGE).toString()
                    user.email = document.data?.get(KEY_EMAIL).toString()
                }
            }
